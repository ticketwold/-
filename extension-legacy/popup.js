...
let lastBtiBetFrame = null; // { tabId, frameId } — ODDS_CHANGED 출처 프레임
let lastBetFailAt = 0;
const BET_FAIL_COOLDOWN_MS = 3000;
...
chrome.runtime.onMessage.addListener((msg, sender) => {
  if (msg.type === 'ODDS_CHANGED') {
    ...
    if (msg.source === 'bti') {
      if (msg.slip?.odds > 1 && !msg.slip?.suspended) {
    ...
    if (botRunning && !betInProgress && !pendingPollTrigger) {
      ...
      polyPollLoop().finally(() => { pendingPollTrigger = false; });
    }
    return;
  }
...
async function ensureBtiContentInFrame(tabId, frameId) {
  try {
    const probe = await probeBtiBetFrameMessage(tabId, frameId);
...
function probeBtiBetFrameMessage(tabId, frameId) {
  return new Promise((resolve) => {
    chrome.tabs.sendMessage(tabId, { type: 'PROBE_BET_FRAME' }, { frameId }, (res) => {
...
function scoreBtiBetProbe(probe, frame, slip) {
  let score = 0;
  if (slip?.odds > 1) score += 100;
  if (probe?.slipOdds > 1) score += 50;
  if (probe?.hasSlip && probe?.hasInput) score += 45;
  else if (probe?.hasSlip) score += 20;
  if (probe?.hasInput) score += 15;
  if (probe?.hasBtn) score += 10;
  if (frame?.frameId > 0) score += 25;
  if (frame?.frameId === 0) score -= 50;
  if (isBtiFrameUrl(frame?.url || '')) score += 15;
  return score;
}
...
async function findBtiBetFrame(tabId, preferFrameId) {
  ...
  for (const frame of ordered) {
    ...
    let probe = await probeBtiBetFrameMessage(tabId, frame.frameId);
    if (!probe) {
...
function isBtiSlipUsable(slip) {
  if (!slip || slip.suspended) return false;
  if (!slip.odds || slip.odds <= 1.01 || slip.odds >= 100) return false;
  return true;
}
...
async function findTabs() {
  ...
  for (const wTab of allWrapperTabs) {
    ...
    const betFrame = await findBtiBetFrame(wTab.id, preferFrame);
      if (betFrame) {
        btiTab = betFrame;
        if (betFrame.frameId > 0) {
          lastBtiBetFrame = { tabId: wTab.id, frameId: betFrame.frameId };
        }
        if (betFrame.slip) cachedBtiSlip = betFrame.slip;
        break;
      }
  }
...
function btiReadSlipFn() {
  ...
  if (/정지된|정지됨|마감|closed|suspended|locked|unavailable|not available/i.test(cardText)) return null;
  for (const sp of card.querySelectorAll('span, div, label')) {
    const t = (sp.textContent || '').trim();
    if (/^정지된$|^정지$|^마감$|^Suspended$|^Closed$/i.test(t)) return null;
  }
...
async function activateTabForBet(tab, delayMs = 150) {
...
function btiSlipStabilizeInject(targetOdds) {
  return new Promise((resolve) => {
    ...
    function getSlipCards() {
      ...
      for (const sel of selectors) {
        for (const el of document.querySelectorAll(sel)) {
          if (seen.has(el)) continue;
          const cn = String(el.className || '');
          if (cn.includes('wrapper') || cn.includes('counter') || cn.includes('PlaceBet') || cn.includes('Tab')) continue;
          seen.add(el);
          cards.push(el);
        }
      }
    ...
    function readCurOdds(card) {
      ...
      const atM = card.textContent.match(/@\s*(\d+(?:\.\d{1,4})?)/);
      if (atM) {
        const n = btiSlipOddsFromTextInject(atM[1]);
        if (n) return n;
      }
      const found = [];
    ...
    function checkReady() {
      ...
      const cards = getSlipCards();
      const hasSlip = cards.length > 0;
      const curOdds = hasSlip ? readCurOdds(cards[0]) : 0;
      const oddsOk = !targetOdds || curOdds === 0 || Math.abs(curOdds - targetOdds) <= 0.08;

      // 베팅하기 버튼은 금액 입력 후에만 나타남 — 슬립+배당만 확인
      if (hasSlip && !hasUpdateNotif && oddsOk) {
        resolve({ ready: true, curOdds, elapsed, frame: location.href });
        return;
      }
      if (elapsed >= 2500 && hasSlip && !hasUpdateNotif) {
        resolve({ ready: true, curOdds, elapsed, forced: true, frame: location.href });
        return;
      }
      ...
      if (elapsed >= MAX_WAIT) {
        resolve({
          ready: false,
          reason: `타임아웃: UpdateNotif=${hasUpdateNotif}, slip=${hasSlip}, curOdds=${curOdds}, target=${targetOdds}`,
          curOdds, elapsed
        });
        return;
      }
...
async function stabilizeBtiSlip(btiTab, targetOdds, maxWaitMs = 1500) {
  if (!btiTab?.id) return { ready: false, reason: '탭 없음' };
  const frameId = btiTab.frameId ?? 0;
  await ensureBtiContentInFrame(btiTab.id, frameId);
  const viaMsg = await new Promise((resolve) => {
    chrome.tabs.sendMessage(
      btiTab.id,
      { type: 'STABILIZE_SLIP', targetOdds, maxWait: maxWaitMs },
      { frameId },
      (res) => resolve(res || null)
    );
  });
  if (viaMsg?.ready) return viaMsg;
  return execAsyncInTab(btiTab, btiSlipStabilizeInject, [targetOdds]);
}

function btiPlaceBetFastInject(amount) {
    ...
    try {
      function slipCount() {
        return Array.from(document.querySelectorAll('[class*="betslip_fe_BetSecondary_bet"]'))
          .filter((el) => {
            const cn = String(el.className || '');
            return !cn.includes('wrapper') && !cn.includes('counter') && !cn.includes('PlaceBet') && !cn.includes('Tab');
          }).length;
      }
      function findInput() {
        return document.getElementById('counter')
          || document.querySelector('input[class*="CounterSecondary_input"], input[placeholder="베팅금"], input[placeholder*="베팅"], input[class*="counter"], input[class*="Counter"]');
      }
      function findBetBtn() {
        const allBtns = Array.from(document.querySelectorAll('button')).filter((b) => !b.disabled);
        for (const btn of allBtns) {
          const t = btn.textContent.trim();
          if (t.includes('배당 수락') || t.includes('배당수락')) return btn;
        }
        for (const btn of allBtns) {
          const t = btn.textContent.trim();
          if ((btn.className || '').includes('sportsbook-Button') && /베팅|수락/i.test(t)) return btn;
        }
        for (const btn of allBtns) {
          if ((btn.className || '').includes('PlaceBetBlock') && !(btn.className || '').includes('clearAll')) return btn;
        }
        return null;
      }
      function clickEl(el) {
      ...
      if (document.querySelector('[class*="UpdateNotification"]')) {
        resolve({ success: false, reason: '배당 업데이트 중' });
        return;
      }
      const input = findInput();
      if (!input) { resolve({ success: false, reason: '금액 input 없음' }); return; }
      if (!slipCount()) { resolve({ success: false, reason: '슬립 없음' }); return; }
      ...
      if (!betBtn) {
        resolve({ success: false, reason: '베팅 버튼 없음' });
        return;
      }
      ...
      for (const b of document.querySelectorAll('button')) {
        ...
        const t = b.textContent.trim();
        if (t === '승인' || t === '확인' || t === 'OK' || t.includes('승인') || t.includes('Confirm')) {
          clickEl(b);
          break;
        }
      }
      await new Promise((r) => setTimeout(r, 450));
      const left = slipCount();
      const okMsg = !!(document.body?.innerText || '').match(/베팅.*(완료|성공|접수)|Bet.*(accepted|placed)/i);
      const ok = left === 0 || okMsg;
      resolve({
        success: ok,
        reason: ok ? null : `슬립 ${left}건 남음`,
        btnText: betBtn.textContent.trim().slice(0, 30)
      });
    } catch (e) {
...
function updateBetCalc(pSlip, bSlip) {
  ...
  if (!pSlip || !bSlip) {
    if (polyMode) {
      pSlip = pSlip || cachedBtiSlip;
      bSlip = bSlip || cachedPolySlip;
    }
  }

  if (polyMode) {
    calcAnchorName.textContent = '텐텐뱃 (10벳)';
    calcOppName.textContent = 'Polymarket';

    if (!pSlip || !bSlip || !pSlip.odds || !bSlip.odds) {
      calcAnchorAmount.textContent = minBetAmount ? `${minBetAmount.toLocaleString()}원` : '-';
      calcOppAmount.textContent = '-';
      if (calcNote) calcNote.textContent = '양쪽 슬립에 배당을 담으면 Polymarket $ 계산';
      return;
    }

    const { btiBet, polyUsd } = calcPolyBetAmounts(pSlip.odds, bSlip.odds);
    calcAnchorAmount.textContent = `${btiBet.toLocaleString()}원`;
    calcOppAmount.textContent = `$${polyUsd.toFixed(2)}`;
    if (calcNote) {
      calcNote.textContent = `텐텐뱃 ${pSlip.odds.toFixed(3)} × Poly ${bSlip.odds.toFixed(3)} · 환율 ${usdtRate.toLocaleString()}원/USD`;
    }
    return;
  }
...
// 텐텐뱃(KRW) 기준 → Polymarket USD (등배당)
function calcPolyBetAmounts(btiOdds, polyOdds) {
  const btiBet = Math.round(minBetAmount);
  const rate = usdtRate > 0 ? usdtRate : 1400;
  const polyUsd = (btiBet * btiOdds) / (polyOdds * rate);
  const polyUsdRounded = Math.max(0.01, Math.round(polyUsd * 100) / 100);
  return { btiBet, polyUsd: polyUsdRounded };
}

async function ensurePolyContentScript(polyTab) {
  if (!polyTab?.id) return false;
  const frameId = polyTab.frameId || 0;
  try {
    const ping = await new Promise((resolve) => {
      chrome.tabs.sendMessage(polyTab.id, { type: 'PING' }, { frameId }, (res) => resolve(res));
    });
...
async function probePolyBetFrame(polyTab) {
  if (!polyTab?.id) return null;
  const frameId = polyTab.frameId || 0;
  return new Promise((resolve) => {
    chrome.tabs.sendMessage(polyTab.id, { type: 'PROBE_POLY' }, { frameId }, (res) => {
      if (chrome.runtime.lastError) resolve(null);
      else resolve(res || null);
    });
  });
...
function polyPlaceBetInject(amountUsd) {
  return new Promise(async (resolve) => {
    try {
      ...
      function isBtnDisabled(btn) {
        ...
        return /disabled|opacity-50|cursor-not-allowed/i.test(btn.className || '');
      }
      function findPanel() {
        ...
        for (const el of document.querySelectorAll('div, section, aside, form')) {
          ...
          if (!/to\s*win/i.test(t) || !/\$\s*[\d,]+/.test(t) || !el.querySelector('input')) continue;
      ...
      function findInput(panel) {
        ...
        for (const inp of root.querySelectorAll('input')) {
          ...
          if (inp.inputMode === 'decimal' || inp.inputMode === 'numeric') score += 8;
      ...
      function isBuySellTab(btn) {
        ...
        if (parent) {
          let n = 0;
          for (const k of parent.querySelectorAll('button, [role="button"], [role="tab"]')) {
      ...
      function scoreBuyBtn(btn, panel) {
        if (!btn || !isVis(btn) || isBtnDisabled(btn) || isBuySellTab(btn)) return -1;
        const t = (btn.textContent || '').replace(/\s+/g, ' ').trim();
        const testId = btn.getAttribute('data-testid') || '';
        if (/sell/i.test(t) && !/buy/i.test(t)) return -1;
        if (t === 'Buy' || t === 'Sell') return -1;
        let score = 0;
      ...
      function findSubmitBtn(panel) {
        const candidates = [];
        for (const btn of (panel || document).querySelectorAll('button, [role="button"]')) {
          const score = scoreBuyBtn(btn, panel);
        ...
        let bestScore = -1;
        for (const btn of (panel || document).querySelectorAll('button, [role="button"]')) {
          if (!isVis(btn) || isBtnDisabled(btn) || isBuySellTab(btn)) continue;
          const t = (btn.textContent || '').replace(/\s+/g, ' ').trim();
          if (/^buy$/i.test(t) || /^sell$/i.test(t)) continue;
      ...
      function findConfirm() {
        for (const btn of document.querySelectorAll('button, [role="button"]')) {
          if (isBtnDisabled(btn)) continue;
          const t = (btn.textContent || '').replace(/\s+/g, ' ').trim();
          if (/^confirm$/i.test(t) || /^place\s*order$/i.test(t) || t === '확인') return btn;
        }
        return null;
      }
      function detectSubmitted(beforeStake, beforePanelText) {
        const confirmBtn = findConfirm();
        if (confirmBtn) return { submitted: true, kind: 'confirm' };
        const stake = readStake(findPanel());
        if (beforeStake > 0 && (!stake || stake < beforeStake * 0.5)) {
          return { submitted: true, kind: 'stake_cleared' };
        }
        const panelNow = findPanel();
        if (panelNow && beforePanelText && panelNow.innerText !== beforePanelText) {
          return { submitted: true, kind: 'panel_change' };
        }
        return { submitted: false };
      }
      const panel = findPanel();
      if (!panel) { resolve({ success: false, reason: '주문 패널 없음' }); return; }
      const input = findInput(panel);
      if (!input) { resolve({ success: false, reason: '금액 input 없음' }); return; }
      ...
      for (let fillTry = 0; fillTry < 4; fillTry++) {
        ...
        const setter = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value')?.set;
        if (setter) setter.call(input, amountStr);
        else input.value = amountStr;
        input.dispatchEvent(new Event('input', { bubbles: true }));
        input.dispatchEvent(new Event('change', { bubbles: true }));
        try {
          input.dispatchEvent(new InputEvent('input', { bubbles: true, data: amountStr, inputType: 'insertText' }));
        } catch (_) {}
        await new Promise((r) => setTimeout(r, 400));
        const stake = readStake(panel);
        if (stake && Math.abs(stake - amount) < 0.05) { amountOk = true; break; }
      }
      if (!amountOk) {
        resolve({ success: false, reason: `금액 입력 실패 ($${amountStr})` });
        return;
      }
      try {
      ...
      let buyBtn = null;
      for (let i = 0; i < 20; i++) {
        buyBtn = findSubmitBtn(panel);
        if (buyBtn && !isBtnDisabled(buyBtn) && !isBuySellTab(buyBtn)) break;
        buyBtn = null;
        await new Promise((r) => setTimeout(r, 80));
      }
      if (!buyBtn) {
        const dbg = Array.from((panel || document).querySelectorAll('button')).filter(isVis).slice(0, 10)
          .map((b) => `"${(b.textContent || '').trim().slice(0, 28)}"`).join(' | ');
        resolve({ success: false, reason: `제출 버튼 없음 | ${dbg}` });
        return;
      }
      const btnText = (buyBtn.textContent || '').trim().slice(0, 60);
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        robustClick(buyBtn);
        await new Promise((r) => setTimeout(r, 450));
        let submitted = detectSubmitted(beforeStake, beforePanelText);
        if (submitted.submitted) {
          ...
          resolve({ success: true, btnText, confirmed: !!confirmBtn, kind: submitted.kind, attempts: attempt + 1 });
          return;
        }
        await new Promise((r) => setTimeout(r, 500));
        submitted = detectSubmitted(beforeStake, beforePanelText);
        if (submitted.submitted || hasDollar) {
          resolve({ success: true, btnText, confirmed: false, kind: 'direct_submit', attempts: attempt + 1 });
          return;
        }
        buyBtn = findSubmitBtn(panel);
        if (!buyBtn || isBtnDisabled(buyBtn) || isBuySellTab(buyBtn)) break;
      }
      resolve({ success: false, reason: `제출 미체결 (${btnText})`, btnText, attempts: 3 });
    } catch (e) {
      resolve({ success: false, reason: e.message });
    }
  });
...
async function verifyBtiSlipCleared(btiTab) {
  return execInTab(btiTab, () => {
    ...
    const okMsg = !!(document.body?.innerText || '').match(/베팅.*(완료|성공|접수)|Bet.*(accepted|placed)/i);
    return { slipLeft: cards.length, okMsg, placed: cards.length === 0 || okMsg };
  });
}

async function placePolyBetViaMessage(polyTab, amountUsd) {
  if (!polyTab) return { success: false, reason: 'Polymarket 탭 없음' };
  const frameId = polyTab.frameId || 0;
  return new Promise((resolve) => {
    chrome.tabs.sendMessage(polyTab.id, { type: 'PLACE_BET', amount: amountUsd }, { frameId }, (res) => {
      if (chrome.runtime.lastError) {
        resolve({ success: false, reason: chrome.runtime.lastError.message });
        return;
      }
...
async function placePolyBetPipeline(polyTab, polyUsd) {
  await ensurePolyContentScript(polyTab);
  let polyResult = await execAsyncInTab(polyTab, polyPlaceBetInject, [polyUsd]);
  if (!polyResult?.success) {
    polyResult = await placePolyBetViaMessage(polyTab, polyUsd);
  }
  if (!polyResult?.success) {
    polyResult = await execAsyncInTab(polyTab, polyPlaceBetInject, [polyUsd]);
  }
  return polyResult;
}

async function placeBtiBetPipeline(resolvedBtiTab, btiBet, bSlip) {
  let btiResult = await execAsyncInTab(resolvedBtiTab, btiPlaceBetFastInject, [btiBet]);
  if (!btiResult?.success) {
    btiResult = await placeBtiBetViaMessage(resolvedBtiTab, btiBet, bSlip);
  }
  if (!btiResult?.success) {
    await ensureBtiContentInFrame(resolvedBtiTab.id, resolvedBtiTab.frameId);
    btiResult = await execAsyncInTab(resolvedBtiTab, btiPlaceBetFastInject, [btiBet]);
  }
  return btiResult;
}
...
async function executePolyBets(btiTab, polyTab, bSlip, pSlip, profit) {
  ...
  try {
    ...
    if (!freshBti) {
      cachedBtiSlip = null;
      addLog('❌ 텐텐뱃 배당 마감/정지 — 구배당 무시, 베팅 취소', 'error');
      lastBetFailAt = Date.now();
      betInProgress = false;
      stopBot();
      addLog('봇 자동 정지 (배당 마감)', 'warn');
      return;
    }
    if (!freshPoly?.odds) {
      addLog('❌ Polymarket 슬립 없음 — 베팅 취소', 'error');
      betInProgress = false;
      return;
    }
    if (!resolvedBtiTab?.id) {
      addLog('❌ 텐텐뱃 BTI iframe 없음', 'error');
      lastBetFailAt = Date.now();
      betInProgress = false;
      stopBot();
      return;
    ...
    const freshProfit = calcProfit(freshBti.odds, freshPoly.odds);
    if (!isProfitInRange(freshProfit)) {
      addLog(`⚠️ 실시간 수익 ${freshProfit.toFixed(2)}% — 구간 밖, 베팅 취소`, 'warn');
      betInProgress = false;
      return;
    }

    const { btiBet, polyUsd } = calcPolyBetAmounts(freshBti.odds, freshPoly.odds);
    lastBtiBetFrame = { tabId: resolvedBtiTab.id, frameId: resolvedBtiTab.frameId };
    addLog(`[v4.3.0] ⚡ 동시베팅: 텐텐뱃 ${btiBet.toLocaleString()}원 (${freshBti.odds}) / Poly $${polyUsd.toFixed(2)} (${freshPoly.odds.toFixed(3)}) → ${freshProfit.toFixed(2)}%`, 'info');
    addLog(`텐텐뱃 프레임 #${resolvedBtiTab.frameId}`, 'info');

    const [btiResult, polyResult] = await Promise.all([
      placeBtiBetPipeline(resolvedBtiTab, btiBet, freshBti),
      placePolyBetPipeline(polyTab, polyUsd)
    ]);

    addLog(`⚡ 동시베팅 응답 ${Date.now() - t0}ms`, 'info');

    if (!btiResult?.success) {
      addLog(`❌ 텐텐뱃 베팅 실패: ${btiResult?.reason || '?'}`, 'error');
      lastBetFailAt = Date.now();
      stopBot();
      addLog('봇 자동 정지 (텐텐뱃 실패)', 'warn');
      if (polyResult?.success) {
        addLog('⚠️ Polymarket만 체결됨 — 수동 확인 필요', 'warn');
      }
      return;
    }
    addLog('✅ 텐텐뱃 베팅 성공', 'success');

    if (!polyResult?.success) {
      addLog(`❌ Polymarket 베팅 실패: ${polyResult?.reason || '?'} (텐텐뱃만 체결)`, 'error');
      addLog('💡 Polymarket 탭에서 Buy 버튼 수동 확인', 'warn');
    } else {
      const confirmNote = polyResult.confirmed ? 'Confirm' : (polyResult.kind || 'Buy');
      addLog(`✅ Polymarket $${polyUsd.toFixed(2)} (${confirmNote}: ${polyResult.btnText || 'Buy'})`, 'success');
      addLog(`🏁 양방 완료! ${freshProfit.toFixed(2)}% (${Date.now() - t0}ms)`, 'success');
    }
  } catch (e) {
...
async function polyPollLoop() {
  ...
  if (bSlip.odds > 20 || pSlip.odds > 20) {
    updateUI(bSlip, pSlip, null);
    return;
  }

  const profit = calcProfit(bSlip.odds, pSlip.odds);
  updateUI(bSlip, pSlip, profit);

  const oddsKey = `${bSlip.odds}_${pSlip.odds}`;
  if (oddsKey !== lastOddsKey) {
    lastOddsKey = oddsKey;
  ...
  if (isProfitInRange(profit)) {
    ...
    if (Date.now() - lastBetFailAt < BET_FAIL_COOLDOWN_MS) return;
    betInProgress = true;
    addLog(`🎯 양방 발견! 수익률 ${profit.toFixed(2)}% (${formatProfitRangeLabel()})`, 'success');
...
async function executeBets(pinSlipTab, opponentTab, pSlip, bSlip, profit, opponentSource) {
  try {
    ...
    if (opponentSource !== 'sbobet') {
      if (btiTab?.id) lastBtiBetFrame = { tabId: btiTab.id, frameId: btiTab.frameId };
      const probe = btiTab.probe
        || await probeBtiBetFrameMessage(btiTab.id, btiTab.frameId)
        || await execInTab(btiTab, probeBtiBetFrameInject);
      ...
      if (!probe?.hasSlip || !probe?.hasInput) {
        addLog('❌ BTI 베팅카트/금액입력 없음 — x10x10s 스포츠 iframe 확인', 'error');
        betInProgress = false;
        return;
      }
