...
def _print_browser_info() -> None:
    print("=== Auto BetSlip Pipeline v1.0.10 ===", flush=True)
    ...
    print("", flush=True)
    print("Chrome 실행 중 → 기존 탭 attach / 없으면 Profile 3 launch_persistent_context", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="BetSlip 자동배팅 파이프라인 v1.0.10")
