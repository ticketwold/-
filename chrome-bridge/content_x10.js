/**
 * x10x10s content script — BetSlip 컨테이너만 읽고 변경 시 service worker에 전송.
 */
(function () {
  const SITE = "x10";
  let lastPayload = "";
  ...
  function readSlip() {
    if (!globalThis.ArbFrameScanner?.readX10Slip) {
      return { ok: false, empty: true, items: [], reason: "scanner-missing" };
    }
    return globalThis.ArbFrameScanner.readX10Slip();
  }
  ...
  function attachObserver() {
    ...
    const slip = document.querySelector(
      '[class*="betslip"], [class*="Betslip"], [class*="bet-slip"], [id*="betslip"]'
    );
    if (!slip) return;
