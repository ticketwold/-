...
function getBetslipRoot(card) {
  ...
  if (input) {
    let root = input.closest('[class*="betslip_fe"], [class*="Betslip"], [class*="betslip"], [class*="slip_fe"], [class*="Slip"]');
    if (!root) root = getInnermostCardContainer(input) || getActiveSlipPanelRoot();
    if (root) return root;
  }
  if (card) {
  ...
  return document.querySelector('[class*="betslip_fe"], [class*="Betslip"]') || getActiveSlipPanelRoot();
}
...
function finalizeBtiOdds(slip) {
  ...
  const fromSlipCard = src === 'slip-display' || src === 'slip-card' || src === 'slip-latched'
    || src === 'in-play-at' || src === 'widgets-x-slip' || src === 'widgets-x-at'
    || src === 'bti-api' || src.includes('bti-api') || slip.fromSlip === true;
  if (fromSlipCard) {
...
function parseSlipFromCard(card) {
  ...
  if (!slipCardVisible(card)) return null;
  if (isInMyBetsPanel(card) || isInsideBetHistory(card)) return null;
...
function getActiveBetslipPanelRoot() {
  ...
  return getActiveSlipPanelRoot();
}

function scoreSlipCard(card) {
  if (!card || !slipCardVisible(card)) return -1;
  if (isInMyBetsPanel(card) || isInsideBetHistory(card)) return -1;
  let score = 0;
  const input = findBtiBetInput();
  if (input) {
...
function resolveTabPanel(tabEl) {
  ...
  const tabKids = [...tabBar.children];
  const idx = tabKids.indexOf(tabEl);
  const bodyKids = [...container.children].filter((c) => c !== tabBar);
  if (idx >= 0 && bodyKids[idx]) return bodyKids[idx];
  return null;
}

function getInnermostCardContainer(anchor) {
  if (!anchor) return null;
  let node = anchor.parentElement;
  while (node) {
    const hasSlipCard = [...node.querySelectorAll('[class*="betInformation__title"], [class*="BetSecondary_bet"]')]
      .some((el) => {
        const card = el.closest('[class*="bet"]') || el.closest('[class*="Bet"]') || el;
        return isCardVisible(card) && !isInsideBetHistory(card);
      });
    if (hasSlipCard) return node;
    node = node.parentElement;
  }
  return null;
}

function getActiveSlipPanelRoot() {
  const input = findBtiBetInput();
  if (input) return getInnermostCardContainer(input);

  const betBtn = findBtiBetButton();
  if (betBtn) return getInnermostCardContainer(betBtn);

  const tabs = findBtiSideTabs();
  const slipTab = tabs.find((t) => t.isSlip && t.active) || tabs.find((t) => t.isSlip);
  if (slipTab) {
    const panel = resolveTabPanel(slipTab.el);
    if (panel) return panel;
  }
...
function queryBoardButtons() {
  ...
  if (!out.length) {
    for (const btn of document.querySelectorAll('button')) {
      if (seen.has(btn) || !boardButtonVisible(btn)) continue;
      const txt = (btn.textContent || '').trim();
...
function getActiveSlipSelectionText() {
  ...
  const cards = getRealSlipCards();
  if (cards.length) {
    return cards[cards.length - 1].querySelector('[class*="betInformation__title"]')?.textContent?.trim() || '';
  }
  for (const btn of queryBoardButtons()) {
    if (!isBoardButtonSelected(btn)) continue;
    const p = parseSelectionButton(btn);
    if (p?.label) return p.label;
    if (p?.rawText) return p.rawText.replace(/\d+\.\d{2,3}\s*$/, '').trim();
  }
  const root = getActiveSlipPanelRoot();
  if (root) {
    const t = root.querySelector('[class*="betInformation__title"], [class*="selectionName"], [class*="SelectionName"]');
...
function readAnyVisibleBoardOdds(hint = {}) {
  ...
  for (const btn of queryBoardButtons()) {
    ...
    if (!(p?.odds > 1.01)) continue;
    if (oppose && (teamNamesMatch(p.label, oppose) || teamNamesMatch(p.rawText, oppose))) continue;
    const selected = isBoardButtonSelected(btn);
    const score = (selected ? 1000 : 0) + p.odds;
    if (!best || score > best.score) {
      ...
      best = {
        ...
        slip: enrichBtiSlip({
          ...
          period: 'ft',
          side: 'home',
          selectionText: p.label || p.rawText || '',
          eventText: evText,
          homeTeam: home,
          awayTeam: away,
          source: selected ? 'board-live' : 'scan-any',
          fromSlip: false
        })
      };
    }
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === 'BTI_DIAG') {
    sendResponse(buildBtiDiagPayload());
    return false;
  }
  if (msg.type === 'PING') {
