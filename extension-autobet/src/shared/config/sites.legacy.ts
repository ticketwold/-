...
export function leg1Label() {
  return SITE_CONFIG.LEG1_LABEL || '텐텐뱃';
}

export function leg1ShortLabel() {
  return SITE_CONFIG.LEG1_SHORT || '텐텐';
}

export function leg2SiteMeta(pref) {
  return SITE_CONFIG.LEG2_SITES[pref] || SITE_CONFIG.LEG2_SITES.bcgame;
}

export function leg2Label() {
  return leg2SiteMeta('bcgame').label;
}

export function leg2ShortLabel() {
  return leg2SiteMeta('bcgame').short;
}

export function leg2PrefLabel(pref) {
  return leg2SiteMeta(pref || 'bcgame').label;
}

export function leg2PrefShort(pref) {
  return leg2SiteMeta(pref || 'bcgame').short;
}

export function formatStrikeFailLine(btiRes, polyRes, leg2Pref) {
...
export function tabEffectiveUrl(tab) {
  if (!tab) return '';
  return String(tab.pendingUrl || tab.url || '').trim();
}

export function leg1TabUrlPatterns() {
  const patterns = [];
...
export function leg2TabUrlPatterns(pref) {
...
export function isSkippableProbeTab(tab) {
...
export function tabLabelForHint(tab) {
...
export function scoreTabTitleForLeg1(tab) {
...
export function scoreTabTitleForLeg2(tab, pref) {
  const title = String(tab?.title || '');
  if (!title) return 0;
  if (pref === 'stake') {
...
export function isLeg1TabUrl(url) {
...
export function scoreLeg1Tab(url, activeId, tabId) {
...
export function isBcGameUrl(url) {
  return hostMatches(url, SITE_CONFIG.BCGAME_HOSTS);
}

export function isStakeUrl(url) {
  return hostMatches(url, SITE_CONFIG.STAKE_HOSTS);
}

export function isBcGameSportsUrl(url) {
...
export function isStakeSportsUrl(url) {
...
export function scoreLeg2Tab(url, activeId, tabId, pref) {
...
export function getWrapperGamecode(url) {
  const m = String(url || '').match(/(?:[?&#]|^)gamecode=(\d+)/i);
  return m ? m[1] : null;
}

export function isX10InPlayShellUrl(url) {
  if (!url) return false;
...
export function isWrapperBtiUrl(url) {
...
export function scoreWrapperBtiTab(url) {
...
export function isInjectableBtiUrl(url) {
...
export function isSportscenterBetslipUrl(url) {
  return /\/api\/sportscenter\/betslip/i.test(String(url || ''));
}

export function sportscenterBetslipPathPrefix(url) {
...
export function resolveBtiSlipApiPaths(url) {
  const paths = [];
...
export function isWidgetsXBetslipUrl(url) {
  return /widgets-x|betslip-root|sportscenter\/betslip/i.test(url || '');
}

export function scoreBtiFrameUrl(url) {
  if (!url) return 0;
...
export function scoreStakeLeg2FrameUrl(url, tabUrl) {
  if (!url) return 0;
...
export function scoreLeg2FrameUrl(url, tabUrl) {
