...
export function bootstrapPanel(scheduler) {
  wirePanelEvents();
  if (scheduler && typeof scheduler.register === 'function') {
    scheduler.register({ id: 'bti-slip-pause', intervalMs: 200, when: () => armedLocal && !haltAutoBet, run: updateBtiSlipPauseState });
    scheduler.register({ id: 'odds-armed', intervalMs: ODDS_POLL_ARMED_MS, when: () => armedLocal, run: refreshOddsLive });
    scheduler.register({ id: 'odds-idle', intervalMs: ODDS_POLL_MS, when: () => !armedLocal && isLeg2Connected(), run: refreshOddsLive });
    scheduler.register({ id: 'amount-sync', intervalMs: AMOUNT_SYNC_ARMED_MS, when: () => ($('preSync')?.checked || armedLocal) && isLeg2Connected(), run: () => liveAmountSync(false) });
    scheduler.register({ id: 'panel-loop', intervalMs: PANEL_LOOP_ARMED_MS, run: panelLoop });
  } else {
    setInterval(() => { if (armedLocal && !haltAutoBet) updateBtiSlipPauseState(); }, 200);
    setInterval(() => { if (armedLocal) refreshOddsLive(); }, ODDS_POLL_ARMED_MS);
    setInterval(() => { if (!armedLocal && isLeg2Connected()) refreshOddsLive(); }, ODDS_POLL_MS);
    setInterval(() => { if (($('preSync')?.checked || armedLocal) && isLeg2Connected()) liveAmountSync(false); }, AMOUNT_SYNC_ARMED_MS);
